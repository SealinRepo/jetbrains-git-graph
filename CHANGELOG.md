...
### 修复
- **用插件的 Push 推送新分支后没有建立 upstream 跟踪关系**：推送执行的一直是裸 `git push origin <分支>:<目标分支>`，从来没带过 `--set-upstream`，新建分支首次推送完本地拿不到跟踪配置，之后 ahead/behind 之类的信息也就无从显示。现在仅在分支尚未配置上游时补 `--set-upstream`；已经有上游的分支保持原配置不动，避免在 Push 对话框里改了推送目标后，已有的跟踪关系被静默改掉
- **当前分支无法重命名**：分支右键菜单的 "Rename..." 之前和 "Delete" 共用 `!isCurrent` 判断条件，导致当前分支看不到这个选项。`git branch -m` 本身就支持重命名当前分支，现已拆成两个独立条件（Delete 依然对当前分支不可用）

### 变更
- **移除未实现的「Mark/Unmark As Favorite」**：这个按钮从来没有真正实现过——后端只弹一条提示、不保存任何状态，界面上也没有任何地方体现收藏与否。现已移除前后端相关逻辑，工具栏保留禁用的星标图标占位

### 其他
- 分支上游的读取统一收口到 `getUpstream()`，改用 `git for-each-ref --format=%(upstream)`：`rev-parse <分支>@{upstream}` 在远程分支被删除、本地远程跟踪 ref 被 prune 之后会误报"无上游"，而只读 `branch.<name>.merge` 配置又会把只配了一半的分支误判成有上游
...
### 修复
- **Git Log 面板侧边栏删除分支按钮点击无反应**：该入口走的 `deleteBranchPrompt` 命令在分支未完全合并时会报错失败，但前端没有 catch 这个 rejected Promise，导致删除静默失败、没有任何提示。现在改为复用分支右键菜单里已有的"失败后询问是否强制删除"流程，并删掉了这条不再使用的重复命令

### 其他
- **统一内联 SVG 图标组件**：把 `webview/src/` 下散落在各文件里的重复/独立图标组件全部收敛到 `shared/components/Icons.tsx` 统一维护，合并了 19 组重复图标，并清理了从未被引用的静态 svg 文件目录
- **移除 shelf patch 剪贴板复制/导入功能**
- **精简重组 ARCHITECTURE.md**：合并重复论证的章节（通信层/缓存/文件监听/读写分离），修正协议文件路径、gitService 子模块拆分等过时描述；`CLAUDE.md` 同步更新目录结构，并去掉已失效的"bridge 协议需手动同步"说明

## [0.4.22] - 2026-09-09

### 修复
- **Commit 面板提交时误带上不该提交的文件**：原来的提交流程是"对勾选的未暂存文件跑 `git add`，再跑一个不带路径参数的裸 `git commit`"，裸 `git commit` 会把 index 里当前的全部内容提交进去——取消勾选一个已暂存文件、或者在终端手动 `git add` 过的文件，都会被一并提交，跟界面上的勾选状态对不上。现在改成提交命令自己带上勾选的路径作为 pathspec（`git commit -- <路径>`），只提交这些路径，不受 index 里其他文件影响

### 其他
- **去掉 Commit 面板的"暂存区"概念**：不再区分 Changes/Staged 两个分组，只有一个 Changes 列表，勾选文件、点提交即可，中间会不会有 `git add` 发生不需要用户关心；右键菜单的 Stage/Unstage 合并成一个固定的 "Add to VCS"；顺手清掉了从未真正生效过的"对比暂存区"diff 选项（该选项此前显示的内容其实一直和"对比工作区"完全一样，只是标题文案不同）
